...
module LikesHelper
  ...
  def like_action(target, current_user=current_user)
    ...
    if target.instance_of?(Comment)
      ...
      if current_user.liked?(target)
        link_to t('shared.stream_element.unlike'), status_message_like_path(target, current_user.like_for(target)), :method => :delete, :class => 'unlike', :remote => true
      else
        link_to t('shared.stream_element.like'), status_message_likes_path(target, :positive => 'true'), :method => :post, :class => 'like', :remote => true
      end
